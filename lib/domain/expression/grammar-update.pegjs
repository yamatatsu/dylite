{{
  ...
  const { AliasAttributeValue } = require('./AttributeValue');
}}
...
AddExpression
  = path:PathExpression _ val:AttributeValue {
...
AttributeValue
  = !ReservedWord head:':' tail:IdentifierPart* {
      const name = head + tail.join('');
      return new AliasAttributeValue(name, options.context.attrValMap);
    }
