...
export const CONFIG_FILE_NAME = "jest-dynamodb-config.js";
export const CONFIG_FILE_NAME_CJS = "jest-dynamodb-config.cjs";
export const CONFIG_FILE_NAME_TS = "jest-dynamodb-config.ts";
...
const readConfig = (): Config => {
	const configFile = findConfigOrError(
		// biome-ignore lint/style/noNonNullAssertion: respected from https://github.com/freshollie/jest-dynalite
...
export const getDynalitePort = (): number => {
	...
	if (Number.isInteger(basePort) && basePort > 0 && basePort <= 65535) {
		return basePort + Number.parseInt(process.env.JEST_WORKER_ID || "1", 10);
	}
